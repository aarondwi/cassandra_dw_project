			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				prepareStmt_tCassandraOutput_1 = connection_tCassandraOutput_1
						.prepare("INSERT INTO "
								...
								+ " (\"id_transaksi\",\"tanggal_pinjam\",\"tanggal_batas\",\"tanggal_kembali\",denda,terbayar,\"nama_jurusan\",fakultas) VALUES (?,?,?,?,?,?,?,?)");
				...
				try {
					...
					while (rs_tDBInput_1.next()) {
						...
						try {
							...
							row4.fdtgl_pinjam = null;
						}

						try {
							row4.fdtgl_batas = dt.parse(row1.fdtgl_batas
									.split(" ")[0]);
						} catch (Exception e) {
							row4.fdtgl_batas = null;
						}

						try {
							row4.fdtgl_kembali = dt.parse(row1.fdtgl_kembali
									.split(" ")[0]);
						} catch (Exception e) {
							row4.fdtgl_kembali = null;
						}

						// number only means a student
						if (row1.fckd_induk.matches("[0-9]+")
								&& row1.fckd_induk.length() == 8) {
							row4.fckd_induk = Integer.valueOf(row1.fckd_induk
									.substring(0, 3));
...
/************************************************************************************************
 * 123439 characters generated by Talend Open Studio for Big Data on the October
 * 2, 2018 7:14:39 PM ICT
 ************************************************************************************************/
