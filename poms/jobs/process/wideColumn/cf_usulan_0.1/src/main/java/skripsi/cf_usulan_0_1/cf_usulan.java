			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				prepareStmt_tCassandraOutput_1 = connection_tCassandraOutput_1
						.prepare("INSERT INTO "
								...
								+ " (\"id_usulan\",\"tgl_usulan\",judul,pengarang,penerbit,\"unique_id\",\"nama_jurusan\",fakultas,\"jenis_usul\",status) VALUES (?,?,?,?,?,?,?,?,?,?)");
				...
				try {
					...
					while (rs_tDBInput_1.next()) {
						...
						row4.tgl = data_checker.computeDate(row1.tgl);

						row4.unique_id = UUID.randomUUID().toString();
						/*
						 * if(row1.jenis_usul!=null){
						 * row4.jenis_usul=row1.jenis_usul; } else {
						 * row4.jenis_usul="u"; }
						 * 
						 * if(row1.kode_status!=null){
						 * row4.kode_status=row1.kode_status; } else {
						 * row4.kode_status=1; }//belum disetujui
						 * 
						 * //null penerbit is unknown if(row1.penerbit==null ||
						 * row1.penerbit.equals("")){ row4.penerbit="unknown"; }
						 * else row4.penerbit=row1.penerbit;
						 * 
						 * //number only means a student
						 * if(row1.fckd_induk.matches("[0-9]+") &&
						 * row1.fckd_induk.length()==8){
						 * row4.fckd_induk=Integer.
						 * valueOf(row1.fckd_induk.substring(0,3)); } else{
						 * row4.fckd_induk=0; }//means an employee
						 * 
						 * //error here means wrong input data, e.g. 15:37.5
						 * SimpleDateFormat dt=new
						 * SimpleDateFormat("MM/dd/yyyy"); try{
						 * row4.tgl=dt.parse(row1.tgl.split(" ")[0]); }
						 * catch(Exception e){ row4.tgl=dt.parse("01/01/0001");
						 * }
						 */
						nb_line_tJavaRow_1++;
...
/************************************************************************************************
 * 162282 characters generated by Talend Open Studio for Big Data on the October
 * 8, 2018 6:54:09 PM ICT
 ************************************************************************************************/
